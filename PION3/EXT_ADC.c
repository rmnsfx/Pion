...
d_COEFF_FILTER FILTER_25000_ONE_SECTION =	{ 1*0.99992757764745543,  -2*0.99992757764745543,  1*0.99992757764745543,  
										 1,  1.9998551329549248, -0.99985517763489695 };       
d_COEFF_FILTER FILTER_25000_TWO_SECTION =	{ 1*0.99984576594563035,  -1*0.99984576594563035,  0,                      
...
void TIM1_UP_IRQHandler(void)
{
		...
		if ((INDEX&DECIMATOR)==0)
		{
				...
				if (DECIMATOR==0x03) ext_adc_VALUE = (SAMPLE[0]+SAMPLE[1]+SAMPLE[2]+SAMPLE[3])>>2;
 ...
 switch (ext_mode_reg)
		   {
			...
			case 0x0800: //канал выборки
						 iir_DEC_FILTER_SET(FILTER_25000_ONE_SECTION, pLPF_1section);
						 iir_DEC_FILTER_SET(FILTER_25000_TWO_SECTION, pLPF_2section);
