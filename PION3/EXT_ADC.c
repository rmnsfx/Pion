...
d_COEFF_FILTER FILTER_SAMPLE_ONE_SECTION =	{ 1*0.99992757764745543,  -2*0.99992757764745543,  1*0.99992757764745543,  1,  1.9998551329549248, -0.99985517763489695 };       
d_COEFF_FILTER FILTER_SAMPLE_TWO_SECTION =	{ 1*0.99984576594563035,  -1*0.99984576594563035,  0,                      1,  0.99969153189126081, 0 };                        

d_FILTER *pHPF_1section, *pHPF_2section,
				 *pLPF_1section, *pLPF_2section, 
			   *pINTEGRAL_1section, *pINTEGRAL_2section;
...
void TIM1_UP_IRQHandler(void)
{
		...
		if ((INDEX&DECIMATOR)==0)
		{
				if (DECIMATOR==0x01) 
				{
					ext_adc_VALUE = (SAMPLE[0]+SAMPLE[1])>>1;
				}
				else if ( (DECIMATOR > 0x01) && (DECIMATOR < 0x04) ) 
				{	
					ext_adc_VALUE = (SAMPLE[0]+SAMPLE[1]+SAMPLE[2]+SAMPLE[3])>>2;
				}
