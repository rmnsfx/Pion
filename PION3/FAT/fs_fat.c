...
/*  FatType  SecClus  SecClus32 NumHeads NumSect NumCyl BootRecSec */
  ...
  { FS_FAT32,    64,      64,      256,    63,    1024,   257 },   /*    8 GB */
  { FS_FAT32,    64,      64,      255,    63,    1024,   257 },   /*   16 GB */
  { FS_FAT32,    64,      64,      255,    63,    1024,   257 }};  /*   32 GB */
...
static BOOL write_label       (const char *label, U32 sect);
static BOOL set_next_clus     (U32 *ptr_clus);
static BOOL get_free_clus     (U32 *ptr_clus);
//static BOOL get_free_clus     (IOB *fcb);
...
static int init_dev (void) {
  ...
  /* Read MBR, return 2 on error. */
  EX(get_mbrec (), 2);

  /* Read Boot Record, return 3 on error. */
  EX(get_bootrec (), 3);

  /* Check if FAT valid, return 4 on error. */
  EX(is_fat_valid (), 4);
  ...
  if (mmc.DataClusCnt < 4085) {
    ...
    if (get_fsinfo () == __FALSE) {
      free_clus = count_free_clus ();
      /* Write corrected FSInfo, return 5 on error. */
      EX(write_fsinfo (), 5);
    }
  }
  return (0);
...
BOOL fat_format (const char *label) {
  ...
  if (iSz == 13) {
    /* Only Flash Card up to 32GB supported. */
    return (__FALSE);
  }
  ...
  /* Wipe hidden sectors. */
  memset (ca.buf, 0, 512);
  for (i = 1; i < mmc.BootRecSec; i++) {
    EX(write_cache (i),__FALSE);
  }
  ...
  if (mmc.FatType == FS_FAT32) {
    free_clus = mmc.DataClusCnt - 1;

    /* Generate FSInfo on FAT32. */
    EX(write_fsinfo (),__FALSE);

    /* Wipe reserved sectors. */
    memset (ca.buf, 0, 512);
    for (i = 2; i < 8; i++) {
      /* Skip Backup Boot Sector */
      if (i == 6) continue; 
      EX(write_cache (i + mmc.BootRecSec),__FALSE);
    }
  }
  ...
  EX(write_cache (sec+mmc.FatSize),__FALSE);

  /* Clear FAT 1 */
  memset (ca.buf, 0, 12);
  for (i = 1; i < mmc.FatSize; i++) {
  ...
  if (*label != 0) {
    EX(write_label (label, sec),__FALSE);
    sec++;
    datSect--;
  }
...
static BOOL write_br (U32 sernum) {
  ...
  EX(write_cache (mmc.BootRecSec),__FALSE);

  /* Backup Boot Sector on FAT32. */
  if (mmc.FatType == FS_FAT32) {
    EX(write_cache (mmc.BootRecSec + 6),__FALSE);
  }
  return (__TRUE);
}
...
static BOOL write_fsinfo (void) {
  /* Construct and write a FSInfo record for FAT32. */

  memset (ca.buf, 0, 512);

  /* Lead Signature */
  set_u32 (&ca.buf[0], 0x41615252);

  /* Struct Signature */
  set_u32 (&ca.buf[484], 0x61417272);

  /* Cluster Free Count */
  set_u32 (&ca.buf[488], free_clus);

  /* Next Free Cluster */
  set_u32 (&ca.buf[492], top_clus);

  /* Trail Signature */
  set_u16 (&ca.buf[510], 0xAA55);

  EX(write_cache (mmc.BootRecSec + mmc.FAT32_FSInfo),__FALSE);
  return (__TRUE);
}
...
U64 fat_free (void) {
  ...
  return ((U64)(free_clus) * mmc.ClusSize);
}
...
static BOOL get_free_clus (U32 *ptr_clus) {
  ...
  for (clus = top_clus; clus < (mmc.DataClusCnt + 2); clus++) {		//!!!!!!!!top_clus
    /* Read a part of FAT table to buffer. */
    sect = get_fat_sect (clus);
    EX(cache_fat (sect),__FALSE);
    switch (mmc.FatType) {
...
static U32 get_fat_sect (U32 clus) {
 ...
 //Юра
BOOL fat_read_data(U32 adr, U8 *buf, U16 cnt) 
{
 U32 sect;
 U16 offset;
 if (cnt>512) return (__FALSE);
 ...
 if (sect==ca.sect)
  if (cnt<=(512-offset)) 
   {
    memcpy(buf,&ca.buf[offset],cnt);
    return (__TRUE);
   }
  else		
   {
    memcpy(buf,&ca.buf[offset],512-offset);
		cnt-=(512-offset);
		buf+=(512-offset); 
		sect++;	  
		offset = 0;
 ...
 //Юра
BOOL fat_write_data(U32 adr, U8 *buf, U16 cnt) 
{
 U32 sect;
 U16 offset;
 if (cnt>512) return (__FALSE);
 ...
 if (sect==ca.sect)
  if (cnt<=(512-offset)) 
   ...
   {
		memcpy(&ca.buf[offset],buf,512-offset);
		cnt-=(512-offset);
		buf+=(512-offset); 
		sect++;	  
		offset = 0;
...
static BOOL cache_fat (U32 sect) {
  ...
  if (fat.dirty == __TRUE) {
    ...
    if (mmc.NumOfFat == 2) {
      EX(mmc_write_sect (fat.sect+mmc.FatSize, fat.buf, 1),__FALSE);
    }
    fat.dirty = __FALSE;
  }
  if (sect == 0) {
...
static BOOL get_fsinfo (void) {
  /* Read FSInfo Record for FAT32. */
  U32 v;

  EX(read_sector (mmc.BootRecSec + mmc.FAT32_FSInfo),__FALSE);

  /* Check Trail Signature. */
  if (get_u32 (&ca.buf[508]) != 0xAA550000) {
    /* Invalid FSInfo Record. */
    return (__FALSE);
  }

  /* Check Lead Signature. */
  if (get_u32 (&ca.buf[0]) != 0x41615252) {
    /* Invalid FSInfo Record. */
    return (__FALSE);
  }

  /* Check Struct Signature. */
  if (get_u32 (&ca.buf[484]) != 0x61417272) {
    /* Invalid FSInfo Record. */
    return (__FALSE);
  }

  /* Read Free Cluster Count. */
  v = get_u32 (&ca.buf[488]);
  if (v > mmc.DataClusCnt) {
    return (__FALSE);
  }   
  free_clus = v;

  /* Read Next Free Cluster. */
  v = get_u32 (&ca.buf[492]);
  if ((v < 2) || (v > mmc.DataClusCnt)) {
    return (__FALSE);
  }   
  top_clus = v;
...
static BOOL rename_entries (const char *new_name, IOB *fcb, U8 type) {
  ...
  if (name_lfn_f) {
    ...
    else if ((((strlen(new_name) + 12) / 13) + 1) > ents) {
      longer_f = 1;
    }
  }
