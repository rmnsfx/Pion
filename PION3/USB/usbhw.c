...
void USB_Init (void) {
 ...
 // RCC->APB2ENR |= (1 << 5);                 /* enable clock for GPIOD */
  ...
  //GPIOC->CRH |= 0x00007000;
  //GPIOC->BRR  = 0x00000800;
...
void USB_Connect (BOOL con) {
  ...
  if (con) {
    RCC->APB1ENR |= (1 << 23); 			   //включаем тактирование
	//CNTR = CNTR_FRES;                         /* Force USB Reset */
    //ISTR = 0;   
    pin_DISC(LOW);//GPIO_ResetBits(GPIOC,GPIO_Pin_11); 
    CNTR = CNTR_RESETM;                     /* USB Reset Interrupt Mask */
  } else {
    //CNTR = CNTR_FRES;                         /* Force USB Reset */
    //ISTR = 0;                                 /* Clear Interrupt Status */
   	pin_DISC(HIGTH);//GPIO_SetBits(GPIOC,GPIO_Pin_11);
    CNTR = CNTR_FRES | CNTR_PDWN;           /* Switch Off USB Device */
	RCC->APB1ENR &= ~(1 << 23);  			//отключаем тактирование
  }
}
