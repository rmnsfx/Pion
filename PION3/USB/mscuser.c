...
volatile BYTE  BulkBuf[MSC_MAX_PACKET]; /* Bulk In/Out Buffer */
...
void MSC_MemoryWrite (void) {
  ...
  //if ((Offset>=((FILEDATA_OFFSET+1)*512))&&((Offset+BulkLen)<=(((FILEDATA_OFFSET+1+8)*512)+512)))	 //разрешаем запись только в файл маршрута
   fat_write_data(Offset,&BulkBuf[0],BulkLen);	  
   /*for (n = 0; n < BulkLen; n++) {
    //if (((Offset + n)>=1536)&((Offset + n)<2048)) 
	// { 
	  //BulkStage = MSC_BS_CSW;
    //USB_SetStallEP(MSC_EP_OUT); 
	  //BulkLen=0;
...
fail: CSW.bStatus = CSW_CMD_FAILED;
      ...
      switch (CBW.CB[0]) {
        ...
        case SCSI_READ10:
          if (MSC_RWSetup()) {
            if ((CBW.bmFlags & 0x80) != 0) {
              //GPIOB->ODR |= LED_RD;       /* Turn On Read LED */
        ...
        case SCSI_WRITE10:
          if (MSC_RWSetup()) {
            if ((CBW.bmFlags & 0x80) == 0) {
              //GPIOB->ODR |= LED_WR;       /* Turn On Write LED */
