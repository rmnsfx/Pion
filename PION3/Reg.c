...
{ 
 ...
 if(num_reg == NUM)
 {	 
		///Если на флеш МК пусто, читаем с SD карты
		if ( (uint16_t)flash_read((uint32_t) 0x807FFC0) == 0xFFFF )	return DATA_REG.Data[num_reg];
 ...
 {	 
	 	///Если на флеш МК пусто, читаем с SD карты
		if ( (uint16_t)flash_read((uint32_t) 0x807FFE0) == 0xFFFF )	return DATA_REG.Data[num_reg];
...
TStatus reg_WRITE(unsigned int num_reg, unsigned int data)
{
 ...
 if (data!=DATA_REG.Data[num_reg]) return _ERR; 
 else return _OK;
} 
...
TStatus reg_LOAD()
{
 ...
 if (PARAMS[i].CopyFlash==1)
 {
		...
		if(i == 23)
		{
			...
			///Если на флеш МК пусто, читаем с SD карты
			if (DATA_REG.Data[i] == 0xFFFF) 
			{
				_reg_READ(adr,&DATA_REG.Data[i],reg_SIZE_REG);
				
				///Сохраняем данные на флеш МК
				flash_unlock();	  
				temp = (uint16_t) flash_read((uint32_t) 0x807FFE0);
				flash_erase_page(0x807FFC0);		
				flash_erase_page(0x807FFE0);		
				flash_write(0x807FFC0, DATA_REG.Data[i]); ///Номер датчика		
		...
		if (i == 25)
		{
			...
			if (DATA_REG.Data[i] == 0xFFFF)
			{
				...
				flash_erase_page(0x807FFC0);		
				flash_erase_page(0x807FFE0);		
				flash_write(0x807FFE0, DATA_REG.Data[i]); ///Коэф. 
